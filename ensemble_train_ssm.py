"""
Multi-seed ensemble training for EMM-DTI with Official Mamba-SSM.

Trains independent runs with different random seeds using the official
Selective StateSpaceModel (mamba-ssm library), averages predictions,
and reports metrics with error bars (mean ± std).

SEPARATE from ensemble_train.py to allow comparison.

Usage:
    python ensemble_train_ssm.py

Seeds: [42, 123, 2024, 456, 789] (same as standard version)
...
# Import SSM versions
from emm_dti.models.emm_dti_ssm import EMMDTI_SSM
...
SEEDS = [42, 123, 2024, 456, 789]  # 5 different seeds
DATA_DIR = "data/human"
CONFIG_PATH = "configs/train_human_ssm.yaml"
ENSEMBLE_DIR = Path("results/ensemble_ssm")
EPOCHS = 200
PATIENCE = 30
BATCH_SIZE = 32
...
def train_seed(seed, seed_index):
    """
    Train one complete run with the given seed using official Mamba-SSM.
    ...
    print(f"\n{'#' * 80}")
    print(f"# SEED {seed_index + 1}/{len(SEEDS)}: seed={seed} [Mamba-SSM]")
    print(f"{'#' * 80}")

    set_seed(seed)

    # ===== Initialize Model with Official Mamba-SSM =====
    model = EMMDTI_SSM(
    ...
    print(f"\n[Seed {seed}] Test Metrics (Mamba-SSM):")
    print(Metrics.format_metrics(test_metrics, prefix=""))

    # Save per-seed results
    seed_results = {
        "seed": seed,
        "model_type": "EMMDTI_SSM (Official Mamba-SSM)",
...
print("\n" + "=" * 80)
print("STARTING ENSEMBLE TRAINING WITH OFFICIAL MAMBA-SSM")
print("=" * 80)
...
print("\n" + "=" * 80)
print("ENSEMBLING PREDICTIONS (Mamba-SSM)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("PER-SEED TEST METRICS (Mamba-SSM)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("ENSEMBLE TEST METRICS (averaged predictions, Mamba-SSM)")
print("=" * 80)
print(Metrics.format_metrics(ensemble_metrics, prefix=""))

print("\n" + "=" * 80)
print("SUMMARY TABLE (Mamba-SSM)")
print("=" * 80)
...
print("=" * 80)
print("\n✅ ENSEMBLE TRAINING COMPLETE (Mamba-SSM)")
print(f"Results: {ENSEMBLE_DIR}/ensemble_results.json")
print("=" * 80)
